...
void keypress(XEvent *e) {
    ...
    for (i = 0; i < LENGTH(keys); i++) {
        if (keysym == keys[i].keysym &&
            ...
            // E.g. Normal functionality case - npresses 0 == keydown immediate
            // fn
            ...
            // Multikey functionality - find index of key, set global, &
            // dispatch
            if ((multikeypendingindex == -1 && multikeyup &&
                 keys[i].npresses == 1) ||
...
void keypresstimerdonesync(int idx) {
    ...
    if (keys[idx].npresses == 1 && !multikeyup) {
        ...
        maxidx = -1;
        for (i = 0; i < LENGTH(keys); i++)
            if (keys[i].keysym == keys[idx].keysym)
                maxidx = i;
        if (maxidx != -1)
            keypresstimerdispatch(MULTIKEY_THRESHOLD_MS_HOLD -
...
void keyrelease(XEvent *e) { multikeyup = 1; }
